...
// Sets scene_ as RenderTarget and Clears the Renderer
...
// Safely destroys the current scene_::ptr_texture instance, then creates a new
...
void RenderingEngine::DrawTexture(SDL_Renderer* r, SDL_Texture *tex, int x, int y, int w, int h) {
	...
	if (w == 0 && h == 0) {
		...
		// the be stored within the destination rectangle below and passed
		// into SDL_RenderCopy.
